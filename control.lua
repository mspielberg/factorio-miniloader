...
local function on_built_miniloader(entity, orientation, tags)
    ...
    if not tags and util.is_output_miniloader_inserter(entity) and fast_replace_miniloader_state and
        fast_replace_miniloader_state.tick == game.tick and fast_replace_miniloader_state.surface == entity.surface and
        fast_replace_miniloader_state.position.x == entity.position.x and fast_replace_miniloader_state.position.y ==
        entity.position.y then
        tags = {
...
local function on_rotated(ev)
    if ev and ev.entity then
        ...
        if util.is_miniloader_inserter(entity) then
            ...
            if miniloader then
                miniloader.rotate {
                    by_player = game.players[ev.player_index]
                }
...
local function on_miniloader_inserter_mined(ev)
    ...
    local inserters = util.get_loader_inserters(entity)
    if util.is_output_miniloader_inserter(entity) and global.split_lane_configuration[entity.unit_number] then
...
local function on_placed_blueprint(ev, player, bp_entities)
    ...
    local surface_area = util.move_box(util.rotate_box(bp_area, ev.direction), ev.position)
    ...
    if blueprint_contained_miniloader then
        ...
        if not global.placed_blueprint_areas then
...
local function check_placed_blueprints_for_miniloaders()
    if not global.placed_blueprint_areas or not next(global.placed_blueprint_areas) then
        return
    end
...
local function on_entity_settings_pasted(ev)
    ...
    if util.is_miniloader_inserter(src) and util.is_miniloader_inserter(dst) then
        local src_loader = src.surface.find_entities_filtered{
            type = "loader-1x1",
            position = src.position
        }[1]
        local dst_loader = dst.surface.find_entities_filtered{
            type = "loader-1x1",
...
local function on_gui_closed(event)
    ...
    if event.gui_type == defines.gui_type.item and event.item and event.item.is_blueprint and
        event.item.is_blueprint_setup() and player.cursor_stack and player.cursor_stack.valid_for_read and
        player.cursor_stack.is_blueprint and not player.cursor_stack.is_blueprint_setup() then
...
local function on_marked_for_deconstruction(ev)
    local entity = ev.entity
    if not (util.is_miniloader(entity) or util.is_miniloader_inserter(entity)) then
        return
    end
    for _, ent in ipairs(entity.surface.find_entities_filtered {
        position = entity.position
    }) do
